 ...
 * command line), so those are refused rather than silently mangled.
 *
...
function planShimSpawn(executable, args = [], options = {}) {
  ...
  for (const arg of args) {
    ...
    if (CMD_UNPASSABLE_ARG.test(arg)) {
      throw new Error(
  ...
  return {
    ...
    // CodeQL flags the concatenation below as a shell command built from input,
    // which is what a /c payload is - cmd.exe takes one command-line string and
    // nothing else. The per-token quoting and the "/%/newline refusals above are
    // the mitigation; CodeQL cannot model cmd.exe's quoting rules to see it.
    args: ['/d', '/s', '/c', `"${command}"`], // codeql[js/shell-command-constructed-from-input]
    verbatim: true
  };
}
